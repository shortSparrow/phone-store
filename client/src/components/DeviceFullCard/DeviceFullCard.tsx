...
const DeviceFullCard: React.FC<DeviceFullCardInterface> = (props) => {
  ...
  const [addToFavotireList, setAddToFavoriteList] = useState<boolean | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    getReguest(
      `/api/${productType}/item/?model_name=${productType}/` + params.model_name
    );
  }, []);

  useEffect(() => {
    const favorite = favoriteDevices.find(
      (item: favoriteDevice) => item._id === currentModel?._id
    );
  ...
  useEffect(() => {
    const cart = cartDeviceList.find(
      (item: cartDevice) => item._id === currentModel?._id
    );
            ...
            <div className="full-device__specifations">
              <div className="full-device__images-wrapper">
                <div className="full-device__small-image-list">
                  {[
                    device.currentDevice.images.main,
                    ...device.currentDevice.images.other,
                  ].map((image: string, index: number) => (
                    <div
                      className={`full-device__small-image ${
                        image === device.currentDevice.bigImage
                          ? "full-device__small-image--active"
                          : ""
                      }`}
                      onClick={() => {
                        const newDevice = {
                ...
                <div className="full-device__big-image">
                  <img
                    src={
                      defaultConstatnts.domain +
                      "/" +
                      device.currentDevice.bigImage
                    }
                    alt={device.title}
              ...
              <div className="full-device__select-block">
                <div className="full-device__available-color-list__wrapper">
                  <p className="card-specification__name full-device__select-titile">
                    Available colors
                  </p>
                  ...
                  <div className="full-device__available-color-list">
                    {device.availabelColor.map((color: string) => (
                      <div
                        className={`full-device__availabe-color-wrapper ${
                          device.currentColor === color
                            ? "full-device__availabe-color-wrapper--selected"
                            : ""
                        }`}
                        key={color}
                      >
                        <div
                          ...
                          onClick={() => {
                            const newDeviceIndex =
                              currentModel?.availabelDevices.findIndex(
                                (model: any) => model.color === color
                              );

                            const newDevice = {
                              ...device,
                              currentDevice: {
                                ...currentModel!.availabelDevices[
                                  newDeviceIndex!
                                ],
                                currentColor:
                                  currentModel!.availabelColor[newDeviceIndex!],
                                bigImage:
                                  currentModel!.availabelDevices[
                                    newDeviceIndex!
                                  ].images.main,
                ...
                <div className="phone-card__price full-device__price">
                  <p className="phone-card__current-price full-device__current-price">
                    {currentModel?.price.current}
                  </p>
                  <p className="phone-card__old-price">
                    {currentModel?.price.old}
                  </p>
                </div>
                ...
                <div className="full-device__short-info">
                  <div className="phone-card__button--wrapper full-device__button-wrapper">
                    <div
                      className={`button__add-cart--wrapper phone-card__add-cart--wrapper ${
                        addToCartList ? "button__add-cart--active" : ""
                      }`}
                    ...
                    <div
                      className="button__favorite--wrapper phone-card__favorite--wrapper"
                      onClick={() => toggleFavoriteDevice(currentModel!)}
                    >
                      <img
                        src={`${
                          addToFavotireList
                            ? "/icons/heart-filed.svg"
                            : "/icons/heart.svg"
                        }`}
                        alt="favorite"
                  ...
                  {device.shortInfo && (
                    <div className="card-specification--list">
                      {Object.entries(device.shortInfo).map(
                        (item: [string, any], index: number) => {
                          const [name, description] = item;

                          return (
                            <div
                              className="card-specification--item"
                              key={name + index}
                            >
                              <p className="card-specification__name">{name}</p>
                              <p className="card-specification__value">
                                {description}
                              </p>
                            </div>
                          );
                ...
                {device.about ? (
                  device.about.map(
                    (
                      ...
                      <div
                        className="full-device__summary__paragraph"
                        key={title + index}
                      >
                        <p className="full-device__summary__title third-title">
                          {item.title}
                        </p>
                        <p className="full-device__summary__description body-text">
                          {item.description}
                        </p>
                      </div>
                    )
                  ...
                  <p className="full-device__summary__title third-title">
                    There aren't any description about this product, we add one
                    in the near future
                  </p>
                )}
              </div>
              <div className="full-device__tech-sepcification">
                <p className="second-title full-device__second-title">
                  Tech specs
                </p>
                <div className="full-device__separate-line"></div>
                <div className="full-device__tech-specifation__list">
                  {Object.entries(device.deviceInfo).map(
                    (item: [string, any], index: number) => {
                      const [name, description] = item;

                      return (
                        <div
                          className="full-device__tech-sepcification__item"
                          key={name + index}
                        >
                          <p className="full-device__tech-sepcification__name card-specification__name">
                            {name}
                          </p>
                          <p className="full-device__tech-sepcification__description card-specification__value">
                            {description}
                          </p>
                        </div>
                      );
...
const mapDispatchToProps = (dispatch: any) => ({
  toggleFavoriteDevice: (device: favoriteDevice) =>
    dispatch(favoritesDevice.toggleFavoriteDevice(device)),
  toggleCartDevice: (device: cartDevice) =>
    dispatch(cartDeviceList.toggleCartDevice(device)),
});
