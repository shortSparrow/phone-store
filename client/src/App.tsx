...
function App() {
  ...
  useEffect(() => {
    ...
    const favoriteDeviceLocalList =
      localStorage.getItem("@favotiteDeviceList") || JSON.stringify([]);
    store.dispatch(setFavoriteDevices(JSON.parse(favoriteDeviceLocalList!)));

    // add chartDeviceList from localStorage to redux
    const cartDeviceLocalList =
      localStorage.getItem("@cartDeviceList") || JSON.stringify([]);
