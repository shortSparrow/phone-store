...
import {
  tabletCardInterface,
  tabletListStateType,
} from "../../interfaces/tabletStateInterface";
...
const TabletsPage: FC = ({
  ...
  const handlePageStructure = (value: number) => {
    ...
    setStructureList({
      ...
      onPage: value,
      pages: Array.from(
        Array(Math.ceil(tabletListState.visible.length / value)),
        (_, i) => i + 1
      ),
  ...
  useEffect(() => {
    if (tabletList.length) {
      ...
      setStructureList({
        ...
        pages: Array.from(
          Array(
            Math.ceil(tabletListState.visible.length / structureList.onPage)
          ),
          (_, i) => i + 1
        ),
        currentVissible: [...tabletListState.visible].slice(
          0,
          structureList.onPage
        ),
        currentPage: 1,
      });
    }
  ...
  const handleVisible = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = [...tabletListState.sorted].filter((phone) =>
      phone.title.toLocaleLowerCase().includes(value.toLocaleLowerCase())
    );
      ...
      <div className="main-limit">
        ...
        <div className="phones-page__select-phones--wrapper">
          <div>
            ...
            <Select
              itemList={selectList}
              setSelectedItem={setSelectedSortValue}
          ...
          <div>
            ...
            <Select
              ...
              defaultSelectIndex={selectitemsOnPageList.findIndex(
                (item) => item.value === structureList.onPage
              )}
              width={128}
            />
          </div>
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  setTabletListState: (tabletListState: tabletListStateType) =>
    dispatch(tablets.tabletListState(tabletListState)),
});
