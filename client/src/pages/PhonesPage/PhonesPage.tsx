...
import {
  phoneCardInterface,
  phoneListStateType,
} from "../../interfaces/phonesInterfaces";
import {
  DeviceCountType,
  DeviceScreenType,
...
const PhonesPage: React.FC<mainPropsInterfaces> = (props) => {
  ...
  const handlePageStructure = (value: number) => {
    ...
    setStructureList({
      ...
      onPage: value,
      pages: Array.from(
        Array(Math.ceil(phoneListState.visible.length / value)),
        (_, i) => i + 1
      ),
  ...
  useEffect(() => {
    if (phoneList.length) {
      ...
      setStructureList({
        ...
        pages: Array.from(
          Array(
            Math.ceil(phoneListState.visible.length / structureList.onPage)
          ),
          (_, i) => i + 1
        ),
        currentVissible: [...phoneListState.visible].slice(
          0,
  ...
  const handleVisible = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = [...phoneListState.sorted].filter((phone) =>
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  setPhoneListState: (phoneListState: phoneListStateType) =>
    dispatch(phones.phoneListState(phoneListState)),
  phoneLoadSuccss: (data: phoneCardInterface[]) =>
    dispatch(phones.phoneSuccess(data)),
});
